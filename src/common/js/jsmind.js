...
; (function ($w) {
    ...
    jm.node = function (sId, iIndex, sTopic, oData, bIsRoot, oParent, eDirection, bExpanded, sTitle, sHandler) {
    ...
    jm.mind.prototype = {
        ...
        add_node: function (parent_node, nodeid, topic, data, idx, direction, expanded, title, handler) {
            if (!jm.util.is_node(parent_node)) {
                ...
                if (!the_parent_node) {
                    ...
                    return this.add_node(the_parent_node, nodeid, topic, data, idx, direction, expanded, title, handler);
                }
            }
            var nodeindex = idx || -1;
            var node = null;
            if (parent_node.isroot) {
                ...
                node = new jm.node(nodeid, nodeindex, topic, data, false, parent_node, d, expanded, title, handler);
            } else {
                node = new jm.node(nodeid, nodeindex, topic, data, false, parent_node, parent_node.direction, expanded, title, handler);
            }
        ...
        node_tree: {
            ...
            _extract_data: function (node_json) {
                ...
                for (var k in node_json) {
                    if (k == 'id' || k == 'topic' || k == 'children' || k == 'direction' || k == 'expanded' || k == 'title' || k == 'handler') {
                        continue;
                    }
                    data[k] = node_json[k];
                }
            ...
            _extract_subnode: function (mind, node_parent, node_json) {
                ...
                var node = mind.add_node(node_parent, node_json.id, node_json.topic, data, null, d, node_json.expanded, node_json.title, node_json.handler);
    ...
    jm.prototype = {
        init: function () {
            ...
            var opts_view = {
                ...
                bounds_width: opts.view.bounds_width,
                bounds_color: opts.view.bounds_color
            };
        ...
        mousedown_handle: function (e) {
            ...
            if (element.nodeName === 'jmnode' && element.className.includes('root')) {
                return;
            }
        ...
        mouseover_handle: function (e) {
            ...
            if (!isexpander && element.className.includes('root')) {
                var i = element.querySelector('i[class^=iconfont]');
                if (i) {
                    i.style.visibility = 'visible';
                }
            }
            if (this.options.custom_event_handles && this.options.custom_event_handles.mouseover) {
                if (typeof this.options.custom_event_handles.mouseover === 'function') {
                    if (element && element.className.includes('jm-card')) {
                        this.options.custom_event_handles.mouseover(element);
                    } else {
                        if (this.options.custom_event_handles.mouseout) {
                            if (typeof this.options.custom_event_handles.mouseout === 'function') {
                                this.options.custom_event_handles.mouseover(element);
                                this.options.custom_event_handles.mouseout(element);
                            }
                        }
                    }
                }
        ...
        click_handle: function (e) {
            ...
            var isexpander = this.view.is_expander(element);
            if (element.nodeName === 'I' && element.className.includes('icontianjiazijiedian')) {
                if (this.options.custom_event_handles && this.options.custom_event_handles.click) {
                    if (typeof this.options.custom_event_handles.click === 'function') {
                        this.options.custom_event_handles.click('root_add_child')
                    }
                }
            }
            if (isexpander) {
                var nodeid = this.view.get_binded_nodeid(element);
                if (!!nodeid) {
                    if (this.options.custom_event_handles && this.options.custom_event_handles.expanderClick) {
                        if (typeof this.options.custom_event_handles.expanderClick === 'function') {
                            this.options.custom_event_handles.expanderClick(this.get_node(nodeid))
                        }
                    }
        ...
        dblclick_handle: function (e) {
            ...
            //     var element = e.target || event.srcElement;
            //     if (element.nodeName === 'JMNODE' && element.className.includes('root') || element.nodeName === 'JMEXPANDER') {
            //         return;
            //     }
            //     var nodeid = this.view.get_binded_nodeid(element);
            //     if (!!nodeid) {
        ...
        _show: function (mind) {
            ...
            this.view.show(this.options.center);
        ...
        add_node: function (parent_node, nodeid, topic, data, title, handler) {
            if (this.get_editable()) {
                var node = this.mind.add_node(parent_node, nodeid, topic, data, title, handler);
                if (!!node) {
        ...
        move_node: function (nodeid, beforeid, parentid, direction) {
            if (this.get_editable()) {
                let custom_event = this.options.custom_event_handles;
                var targetNode = this.mind.move_node(nodeid, beforeid, parentid, direction);
                if (custom_event && custom_event.beforeMoved && typeof custom_event.beforeMoved === 'function') {
                    custom_event.beforeMoved({
                        targetNode, 
                        nodeid, 
                        beforeid, 
                        parentid, 
                        ...
                        success: (node) => {
                            if (!!node) {
                                ...
                                if (custom_event && custom_event.moved && typeof custom_event.moved === 'function') {
                                    custom_event.moved({nodes:this.mind.nodes})
                                }
                            }
                        }, 
                        error: (err) => {}
    ...
    jm.layout_provider.prototype = {
        ...
        _layout_offset_subnodes: function (nodes) {
            ...
            while (i--) {
                ...
                //     layout_data.offset_x += node._data.view.width;
                // }
                layout_data.offset_x += node._data.view.width;
        ...
        get_parent_count: function (node) {
            var obj = {
                count: 0,
        ...
        get_node_point: function (node) {
            ...
            //logger.debug(offset_p);a
            var p = {};
            var parent_count = this.get_parent_count(node);
            // p.x = offset_p.x + view_data.width * (node._data.layout.direction - 1) / 2;
            if (node.isroot) {
                p.x = offset_p.x + view_data.width * (parent_count > 1 ? (parent_count - 1) : parent_count);
            } else {
                p.x = this.jm.view.get_parent_node_offset_right(node);
                // p.x = (offset_p.x - 48) + view_data.width * (parent_count > 1 ? (parent_count - 1) : parent_count);
            }
        ...
        get_expander_point: function (node) {
            ...
            if (node._data.layout.direction == jm.direction.right) {
                ex_p.x = this.jm.options.part.interval * this.get_parent_count(node) + this.jm.options.part.start;
    ...
    jm.graph_canvas = function (view) {
        this.view = view;
        this.opts = view.opts;
        this.l_canvas = $c('canvas');
        this.canvas_lctx = this.l_canvas.getContext('2d');
    ...
    jm.graph_canvas.prototype = {
        ...
        line_element: function () {
            return this.l_canvas;
        },
        ...
        draw_bounds: function (opts) {
            if (opts.part && opts.part.count) {
                this.l_canvas.width = document.body.clientWidth;
                this.l_canvas.height = this.size.h;
                var lctx = this.canvas_lctx;
                lctx.setLineDash([5,3]);
                lctx.lineWidth = this.opts.bounds_width;
                lctx.lineCap = 'butt';
                this.bounds_line(lctx, opts);
            }
        },

        bounds_line: function (lctx, opts) {
            var len = opts.part.count;
            var width = this.l_canvas.width;
            lctx.beginPath();
            lctx.moveTo(opts.part.start, 0);
            lctx.lineTo(opts.part.start, this.e_canvas.height);
            --len;
            while (len--) {
                lctx.moveTo(opts.part.interval * (opts.part.count - (len + 1)) + opts.part.start, 0);
                lctx.lineTo(opts.part.interval * (opts.part.count - (len + 1)) + opts.part.start, this.l_canvas.height);
            }
            lctx.strokeStyle = this.opts.bounds_color;
            lctx.stroke();
        },
        ...
        _line_to: function (ctx, x1, y1, x2, y2, node) {
            var view = node._data.view;      
            ctx.beginPath();
            ctx.moveTo(view.element.offsetLeft, y1);
            ctx.lineTo(view.element.offsetLeft - 12, y1);
            ctx.lineTo(view.element.offsetLeft - 12, y2);
            ctx.lineTo(view.element.offsetLeft - 24, y2);
            ctx.stroke();
        }
    };
    ...
    jm.view_provider.prototype = {
        init: function () {
            ...
            this.e_panel.appendChild(this.graph.line_element());
        ...
        get_style(element, cssPropertyName){
        ...
        init_nodes_size: function (node) {
            var view_data = node._data.view;
            var parent = node.isroot ? null : node.parent;
            var width = !parent ? 
                        parseFloat(this.get_style(view_data.element, 'width')) : 
                        parseFloat(this.get_style(parent._data.view.element, 'width'));
            var paddingLeft = !parent ? 
                              parseFloat(this.get_style(view_data.element, 'paddingLeft')) : 
                              parseFloat(this.get_style(parent._data.view.element, 'paddingLeft'));
            var paddingRight = !parent ? 
                               parseFloat(this.get_style(view_data.element, 'paddingRight')) : 
                               parseFloat(this.get_style(parent._data.view.element, 'paddingRight'));

            view_data.parent_width = !parent ? 
                              view_data.element.clientWidth :
        ...
        create_node_element: function (node, parent_node) {
            ...
            d_e.className = 'iconfont ' + (node.expanded ? 'iconjianhao' : 'iconjiahao');
            parent_node.appendChild(d_e);
            view_data.expander = d_e;
            if (!!node.topic) {
                if (this.opts.support_html) {
                    if (!node.isroot) {
                        if (this.jm.options.customInnerHTML) {
                            $h(d, this.jm.options.customInnerHTML.renderHTML(node));
                        }
                    } else {
                        $h(d, node.topic);
                    }
            ...
            if (node.isroot) {
                var i = $c('i');
                i.className = 'iconfont icontianjiazijiedian';
                i.setAttribute('title', '插入子节点');
        ...
        update_node: function (node) {
            ...
            let custom_event = this.jm.options.custom_event_handles;
            if (custom_event && custom_event.updateNode && typeof custom_event.updateNode === 'function') {
                custom_event.updateNode({node})
            }
        },
        ...
        _show: function () {
            ...
            this.graph.draw_bounds(this.jm.options);
        ...
        get_parent_node_offset_right: function (node) {
            var opts = this.jm.options;
            if (node.parent && node.parent.isroot) {
                return opts.part.start;
            }
            if (node.parent && !node.parent.isroot) {
                var parent_count = this.layout.get_parent_count(node);
                return opts.part.interval * ((opts.part.count - 1) - (opts.part.count - parent_count)) + opts.part.start;
            } else {
                return null;
            }
            // console.log(node.id, node._data.view.width, node._data.view.element.offsetLeft)
            // console.log(this.get_style(node._data.view.element, 'left'))
        },

        get_sibling_count: function (node) {
            if (!node || !node.parent) {
                return 0;
            } else {
                if (node.parent.children && node.parent.children.length) {
                    var index = node.parent.children.findIndex(item => {
                ...
                return node.parent.children ? (node.parent.children.length - 1) : 0;
            }
        },
        ...
        show_nodes: function () {
            ...
            for (var nodeid in nodes) {
                ...
                // node_element.style.left = (node.isroot ? (_offset.x + p.x) : (_offset.x + p.x + 20)) + 'px';
                if (node.isroot) {
                    node_element.style.left = (_offset.x + p.x + 26) + 'px';
                } else {
                    var right = this.get_parent_node_offset_right(node);
                    var node_offset = this.layout.get_node_offset(node);
                    node_element.style.left = (right + 30) + 'px';
                }
                node_element.style.top = (_offset.y + p.y) + 'px';
                node_element.style.display = '';
                node_element.style.visibility = 'visible';
                if (node.isroot || (!node.isroot && node.children.length > 0)) {
                    // expander_text = node.expanded ? '-' : '+';
                    expander.className = 'iconfont ' + (node.expanded ? 'iconjianhao' : 'iconjiahao');
                    p_expander = this.layout.get_expander_point(node);
                    if (node.isroot) {
                        expander.style.left = '68px';
                    } else {
                        expander.style.left = (_offset.x + p_expander.x - 8) + 'px';
                    }
