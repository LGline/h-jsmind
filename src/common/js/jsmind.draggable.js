...
(function ($w) {
    ...
    jsMind.draggable.prototype = {
        ...
        _create_shadow: function () {
            ...
            var selected = this.jm.mind.selected;
            if (selected && opts.customInnerHTML[selected.topic]) {
                var s = $d.createElement('jmnode');
                // var el = selected._data.view.element;
                s.innerHTML = opts.customInnerHTML[selected.topic]
        ...
        dragstart: function (e) {
            ...
            if (!!nodeid && node) {
                if (!node.isroot) {
                    this.reset_shadow(node._data.view.element);
        ...
        drag: function (e) {
            ...
            if (this.capture) {
                ...
                this.shadow.style.left = (px + this.offset_x - 30) + 'px';
                this.shadow.style.top = (py + this.offset_y - 30) + 'px';
                clear_selection();
            }
        },
        ...
        move_node: function (src_node, target_node, target_direct) {
            ...
            if (jsMind.node.inherited(src_node, target_node)) {
                ...
                if (custom_event && custom_event.beforeMoved && typeof custom_event.beforeMoved === 'function') {
                    custom_event.beforeMoved({isInherited: true})
                }
            }
